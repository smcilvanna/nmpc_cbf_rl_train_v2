...
def plotSimdata2(simdata,simdata2,env):
    ...
    # XY position plot
    ax1.scatter(x2, y2,s=3, label="td3-episode",    c='grey',   alpha = 0.5)
    ax1.plot(x, y,          label="sac-step",       c='orange',  lw = 1)      # Plots y versus x as a line
    # ax1.scatter(x3, y3,s=4, label="cbf-0.5",    c='green',  alpha=0.5)
    ax1.add_patch(Circle(simdata[-1,0:2], 0.55, color='black', alpha=0.9,))
    ax1.add_patch(Circle(target[0:2], 0.2, color='green', alpha=0.9))
    for i in range(ob.shape[0]):
    ...
    # mpc time plot
    ax2.plot(t2,mpct2,  label=f"{round(mpcave2*1000,1)} ms average td3",    c='grey')
    ax2.plot(t,mpct,    label=f"{round(mpcave*1000,1)} ms average sac",     c='orange')
    # ax2.plot(t3,mpct3,  label=f"{round(mpcave3*1000,1)} ms average 0.5",    c='green')
    ax2.set_ylim(10,100)
    ax2.legend(fontsize="small")
    ...
    # ax3b.plot(t, a, label="action", color='orange')
    # ax3b.set_ylim(0, 1)
    ax3.plot(t,a,   label="sac",    c='orange')
    ax3.plot(t2,a2, label="td3",    c='grey')
    # ax3.plot(t3,a3, label="cbf-0.5",     c='green')
    ax3.set_ylim(0,1)
    ax3.legend(fontsize="small")
...
if __name__ == "__main__":
    obs = 4.8
    # Load datafiles
    with open(f'test_data/data-{obs}-sac.pkl', 'rb') as f: 
        simdata = pickle.load(f)

    with open(f'test_data/data-rl-cbf1-{obs}.pkl', 'rb') as f: 
        simdata2 = pickle.load(f)

    with open(f'test_data/test-1_obs-{obs}.pkl', 'rb') as f: 
        map = pickle.load(f)

    plotSimdata2(simdata,simdata2,map)
